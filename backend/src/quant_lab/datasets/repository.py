...
class DatasetRepository:
    ...
    def list_files(self, version_id: str) -> tuple[DatasetFileModel, ...]:
        with Session(self._engine) as session:
            files = tuple(
                session.scalars(
                    select(DatasetFileModel)
                    .where(DatasetFileModel.dataset_version_id == version_id)
                    .order_by(DatasetFileModel.relative_path)
                )
            )
            for item in files:
    ...
    def list_all_versions(self) -> tuple[DatasetVersionModel, ...]:
        with Session(self._engine) as session:
            versions = tuple(
                session.scalars(
                    select(DatasetVersionModel).order_by(
                        DatasetVersionModel.dataset_id, DatasetVersionModel.version
                    )
                )
            )
        ...
        with Session(self._engine) as session:
            ...
            session.add(
                PublicationAuditModel(
                    ...
                    dataset_id=dataset_id,
                    dataset_version_id=version_id,
                    expected_preview_fingerprint=version.source_preview_fingerprint,
                    actual_preview_fingerprint=version.source_preview_fingerprint,
                    publication_fingerprint=publication_fingerprint,
                    publication_config_json=config_json,
                    confirm_warnings=confirm_warnings,
                    result="PUBLISHED",
        ...
        *,
        ...
        with Session(self._engine) as session:
            ...
            if (
                version is None
                or batch is None
                or version.status == DatasetVersionStatus.PUBLISHED.value
            ):
                return
            ...
            session.add(
                PublicationAuditModel(
                    ...
                    dataset_version_id=version_id,
                    expected_preview_fingerprint=version.source_preview_fingerprint,
                    actual_preview_fingerprint=None,
