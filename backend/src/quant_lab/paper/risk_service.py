...
class PaperRiskService:
    ...
    def evaluate_intent(
        ...
        *,
        ...
        existing = self.repository.get_risk_decision_by_intent(intent_id)
        if existing is not None:
            return existing

        intent = self.repository.get_intent(intent_id)
        session = self.repository.get_session(intent.paper_session_id)
        account = self.repository.get_account(session.paper_account_id)
        policy = self._active_policy(account.id)
        version = self.repository.get_latest_policy_version(policy.id)

        latest_snapshot = self.repository.get_latest_snapshot(account.id)
        ...
        position = self._position(account.id, intent.instrument_id)
        position_snapshot = (
            RiskPositionSnapshot(
        ...
        market_context = RiskMarketContext(
            ...
            session_date=session.current_session_date or intent.signal_session_date,
        )
        ...
        open_order_count = self.repository.count_open_orders(session.id)
    ...
    def _active_policy(self, account_id: str) -> PaperRiskPolicyModel:
        for policy in self.repository.list_risk_policies(account_id):
            if policy.status == "ACTIVE":
                return policy
        raise PaperError("RISK_POLICY_NOT_FOUND", "账户无ACTIVE风控策略")

    def _position(self, account_id: str, instrument_id: str) -> PaperPositionModel | None:
    ...
    def _persist(
        ...
        *,
        ...
        with Session(self.repository.engine) as session:
            ...
            if evaluation.freeze_required:
                account_model = session.get(PaperAccountModel, account.id)
                if (
                    account_model is not None
                    and account_model.status != PaperAccountStatus.FROZEN.value
                ):
                    account_model.status = PaperAccountStatus.FROZEN.value
